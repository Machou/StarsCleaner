(() => {
	...
	// ----------------------------------------
	// Rendering (chunked, non-blocking)
	// ----------------------------------------
	...
	const render = () => {
		...
		// compute visible list once (fast + count accurate)
		const visibleRepos = [];
		for (const r of repos) {
			if (passesFilters(r)) visibleRepos.push(r);
		}

		// clear DOM now (instant feedback)
		tbody.innerHTML = "";
		updateCountWith(visibleRepos.length);

		// progressively append rows (prevents mobile freeze)
		let i = 0;

		const pump = () => {
			if (job !== renderJobId) return; // cancelled by newer render
	...
	document.addEventListener("DOMContentLoaded", async () => {
		...
		// Hide repo on click (robust: uses full_name, not index)
		const tbody = document.getElementById("repo-body");
		if (tbody) {
